...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function showRestrictedWarning(attemptedUrl) {
        ...
        if (warningBlockedUrl)
            warningBlockedUrl.textContent = attemptedUrl || "Unknown Address";
        if (warningActiveClass)
            warningActiveClass.textContent = classLabel;
    ...
    function navigateWebview(wv, targetUrl) {
        if (!wv || !targetUrl)
            return;
        try {
    ...
    if (warningReturnHomeBtn) {
        warningReturnHomeBtn.addEventListener("click", () => {
            if (restrictedWarningModal)
                restrictedWarningModal.style.display = "none";
    ...
    function syncHomePortalData(webview) {
    ...
    function updateTabFavicon(iconSpan, faviconUrl, currentUrl) {
        ...
        iconSpan.className = "tab-icon tab-icon-web";
        let img = iconSpan.querySelector("img.tab-favicon");
        if (!img) {
            iconSpan.innerHTML = "";
    ...
    function updateTabIdentity(tab, url, pageTitle, faviconUrl) {
    ...
    function addNewTab(url) {
        ...
        const newTab = { id: tabId, webview, tabElement: tabItem, titleSpan, iconSpan };
        tabs.push(newTab);
        ...
        webview.addEventListener("dom-ready", () => {
            ...
            if (savedZoom !== 1.0 && typeof webview.setZoomFactor === "function") {
                try {
        ...
        webview.addEventListener("did-stop-loading", () => {
            if (loadingBar) {
                ...
                setTimeout(() => {
                    if (loadingBar.className === "finish")
    ...
    function handleRefresh() {
        ...
        if (loadingBar) {
            loadingBar.className = "loading";
            setTimeout(() => { if (loadingBar.className === "loading")
                loadingBar.className = "finish"; }, 400);
            setTimeout(() => { if (loadingBar.className === "finish")
                loadingBar.className = ""; }, 800);
        }
        const currentUrl = wv.getURL?.() || wv.getAttribute("src") || "";
        const isHome = !currentUrl || currentUrl.includes("home.html") || currentUrl.startsWith("file:");
        if (isHome) {
            ...
            if (typeof wv.executeJavaScript === "function") {
                wv.executeJavaScript(`window.location.reload();`).catch((err) => {
    ...
    function refreshSettingsData() {
        ...
        if (settingsClassName)
            settingsClassName.textContent = name || "Not Enrolled";
        if (settingsClassCode)
            settingsClassCode.textContent = code || "NONE";
        ...
        if (settingsWhitelistChips) {
            if (whitelist.length === 0) {
                ...
                settingsWhitelistChips.innerHTML = whitelist.map(site => `<span class="settings-chip">🌐 ${escapeHtml(site)}</span>`).join("");
            }
        }
        ...
        if (downloadPathInput) {
            if (savedDownloadPath) {
                ...
                electron_1.ipcRenderer.invoke("get-default-downloads-path").then(defPath => {
    ...
    if (settingsTestConnBtn) {
        settingsTestConnBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                const result = await electron_1.ipcRenderer.invoke("firebase-test-connection");
    ...
    if (settingsClearCacheBtn) {
        settingsClearCacheBtn.addEventListener("click", async () => {
            try {
                ...
                const res = await electron_1.ipcRenderer.invoke("clear-browser-session-data");
                ...
                if (settingsCacheFeedback) {
                    ...
                    setTimeout(() => {
                        if (settingsCacheFeedback)
                            settingsCacheFeedback.style.display = "none";
                    }, 4000);
                }
            }
            catch (err) {
    ...
    function applyZoomToAllWebviews(factor) {
    ...
    function activateClassSession(code, name, whitelistedWebsites) {
        ...
        if (classGateModal)
            classGateModal.style.display = "none";
        if (classInfoBadge)
            classInfoBadge.style.display = "flex";
        if (activeClassText)
            activeClassText.textContent = `Class: ${name} (${code})`;
    ...
    async function submitClassCode(codeToVerify) {
        ...
        try {
            const res = await electron_1.ipcRenderer.invoke("firebase-verify-class-code", { classCode: code });
            if (res.success && res.found) {
                setGateStatus(res.message, "success");
                setTimeout(() => {
                    activateClassSession(code.toUpperCase(), res.classData?.name || code.toUpperCase(), res.classData?.whitelistedWebsites);
                }, 600);
            }
            else {
    ...
    async function resumePreviousClass() {
        ...
        if (tabs.length > 0) {
            ...
            if (prevName)
                localStorage.setItem("active_class_name", prevName);
            ...
            if (classInfoBadge)
                classInfoBadge.style.display = "flex";
            if (activeClassText)
                activeClassText.textContent = `Class: ${prevName} (${prevCode})`;
            return;
        }
    ...
    switchClassBtn?.addEventListener("click", () => {
        ...
        if (currentCode) {
            ...
            if (currentName)
                localStorage.setItem("previous_class_name", currentName);
            ...
            if (classGateClose)
                classGateClose.style.display = "flex";
            if (classGateCancelBtn)
                classGateCancelBtn.style.display = "block";
        }
        else {
            updatePreviousClassBox();
            if (classGateClose)
        ...
        if (classGateStatus)
            classGateStatus.style.display = "none";
    });
